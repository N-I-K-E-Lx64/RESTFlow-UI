...
export const VariablesFieldArray = () => {
	...
	return (
		<Box sx={{ display: "flex", flexDirection: "column" }}>
			{ fields.map((field, index) => (
				<Box key={index} sx={{ display: "flex", flexDirection: "row", '& .MuiTextField-root': { width: '25ch', m: 1 } }}>
					<Controller
			...
			<IconButton color="primary" aria-label="add variable" onClick={() => append({ name: "", type: 0 })}>
				<Add />
			</IconButton>
		</Box>
	);
