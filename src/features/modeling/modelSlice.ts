...
export const modelSlice = createSlice({
  ...
  reducers: {
    ...
    removeElement: (state, action: PayloadAction<Element>) => {
      ...
      const index = state.model.elements.findIndex(
        (element: Element) => element.id === id
      );
      if (index !== -1) {
        state.model.elements.splice(index, 1);
        connectors.forEach((connectorId) => {
          const connIndex = state.model.connectors.findIndex(
            (connector: Connector) => connector.id === connectorId
          );
          if (connIndex !== -1) {
    ...
    updateConnector: (state, action: PayloadAction<ConnectorUpdate>) => {
      ...
      const index = state.model.connectors.findIndex(
    ...
    removeConnector: (state, action: PayloadAction<Connector>) => {
      ...
      const sourceIndex = state.model.elements.findIndex(
        (element: Element) => element.id === source
      );
      const targetIndex = state.model.elements.findIndex(
        (element: Element) => element.id === target
      );
      if (sourceIndex !== -1 && targetIndex !== -1) {
        const sourceConnectorIndex = state.model.elements[
          sourceIndex
        ].connectors.findIndex((connectorId) => connectorId === id);
        const targetConnectorIndex = state.model.elements[
          targetIndex
        ].connectors.findIndex((connectorId) => connectorId === id);
        state.model.elements[sourceIndex].connectors.splice(
    ...
    updateTask: (state, action: PayloadAction<Task>) => {
      ...
      const elementIndex = state.model.elements.findIndex(
        (element: Element) => element.id === id
      );
      if (index !== -1) {
